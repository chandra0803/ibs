...
@Service("tailogService")
public class TailogServiceImpl implements TailogService,Serializable {
...
log.info("Loading CONFIGURATION_PROPERTIES...");
       ...
       if(updateDate!=null) {
		...
		String url=prop.getProperty("tailog.webservice.url");
		...
		*/
		...
		if(response!=null) {
			...
			if(response.getStatusCodeValue()==200) {
	            ...
	            String updateQuery = "UPDATE airlinkdatabase.update_table set updated_after = '" + stdToDate +"' where id=1"; 
	            log.info("Update Query:"+updateQuery);
	            x = stmt.executeUpdate(updateQuery); 
				}else {
					log.info("Inserting with stdToDate");
					String insertQuery = "INSERT INTO airlinkdatabase.update_table (id,updated_after) VALUES (1,'" + stdToDate +"')"; 
					...
					String updateQuery = "UPDATE airlinkdatabase.update_table set updated_after = '" + stdFromDate +"' where id=1"; 
		            log.info("Update Query:"+updateQuery);
		            x = stmt.executeUpdate(updateQuery);
					}else {
						log.info("Inserting with stdFromDate");
						String insertQuery = "INSERT INTO airlinkdatabase.update_table (id,updated_after) VALUES (1,'" + stdFromDate +"')"; 
			            log.info("Insert Query:"+insertQuery);
			            stmt.execute(insertQuery);
			            x=1;
					}
	...
	*/
	...
	public ResultSet selectUpdatedAfterValue(Connection  conn) {
		...
		try {
		  ...
		  String selectQuery = "select * from airlinkdatabase.update_table where id=1"; 
	        ResultSet rs = null;
			try {
